...
workflow PhasedPanelEvaluation {    # TODO change name later, easier to share configs for now
    ...
    if (do_short_read_stages) {
        ...
        scatter (i in range(length(LeaveOutEvaluation.glimpse_unphased_vcf_gzs))) {
            call NaivelyPhase as GLIMPSENaivelyPhased { input:
                vcf_gz = LeaveOutEvaluation.glimpse_unphased_vcf_gzs[i],
                vcf_gz_tbi = LeaveOutEvaluation.glimpse_unphased_vcf_gz_tbis[i],
                docker = kage_docker,
                monitoring_script = monitoring_script
            }
        }

        # merge naively phased GLIMPSE VCFs
        call Ivcfmerge as GLIMPSENaivelyPhasedMergeAcrossSamples { input:
            vcf_gzs = GLIMPSENaivelyPhased.naively_phased_vcf_gz,
            vcf_gz_tbis = GLIMPSENaivelyPhased.naively_phased_vcf_gz_tbi,
            sample_names = leave_out_sample_names,
            output_prefix = output_prefix + ".glimpse.naively_phased.merged",
        ...
        call VcfdistAndOverlapMetricsEvaluation.VcfdistAndOverlapMetricsEvaluation as EvaluateGLIMPSENaivelyPhased { input:
            ...
            truth_vcf = vcfdist_truth_vcf,
            truth_vcf_idx = vcfdist_truth_vcf_idx,
            eval_vcf = GLIMPSENaivelyPhasedMergeAcrossSamples.merged_vcf_gz,
            eval_vcf_idx = GLIMPSENaivelyPhasedMergeAcrossSamples.merged_vcf_gz_tbi,
            region = evaluation_chromosomes_regions_arg,
            reference_fasta = reference_fasta,
            reference_fasta_fai = reference_fasta_fai,
            vcfdist_bed_file = vcfdist_bed_file,
