...
workflow LeaveOutEvaluation {
    ...
    scatter (j in range(length(leave_out_sample_names))) {
        ...
        call KAGEPlusGLIMPSECase.KAGEPlusGLIMPSECase as KAGEPlusGLIMPSECase {
            input:
    ...
    output {
        Array[File] kage_vcf_gzs = KAGEPlusGLIMPSECase.kage_vcf_gz
        Array[File] kage_vcf_gz_tbis = KAGEPlusGLIMPSECase.kage_vcf_gz_tbi
        Array[File] glimpse_unphased_vcf_gzs = KAGEPlusGLIMPSECase.glimpse_unphased_vcf_gz
                ...
                for allelic in ['biallelic+multiallelic', 'multiallelic', 'biallelic']:
                    ...
                    for i, (filter_name_i, is_v_i) in enumerate(is_altfreq_v):
                        for j, (filter_name_j, is_v_j) in enumerate(is_altlen_v):
                            ...
                            is_eval_v = ~is_any_missing_truth_v & is_v_i & is_v_j & is_allelic_v & is_context_v
...
# old version that doesn't set missing to hom-ref in case
task CalculateMetricsWithoutMissing {
    input {
        File case_vcf_gz        # bi+multi split
        File case_vcf_gz_tbi
        File truth_vcf_gz       # bi+multi split
