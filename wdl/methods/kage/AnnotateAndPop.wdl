...
workflow AnnotateAndPop {
    input {
        Array[File] collisionless_vcf_gzs       # whole-genome, per-batch
    ...
    scatter (i in range(length(collisionless_vcf_gzs)))
    {
        scatter (j in range(length(chromosomes))) {
            call AnnotateAndPop as ChromosomeAnnotateAndPop {
                input:
                    collisionless_vcf_gz = collisionless_vcf_gzs[i],
...
task AnnotateAndPop {
    ...
    Int disk_size_gb = 3 * ceil(size([collisionless_vcf_gz, posteriors_vcf_gz] , "GB"))
